...
export class TimelineComponent implements OnChanges {
  ...
  @Input() public timeformat = 'h:mm:ss a';
  @Input() public periodTitles = true;
  @Input() public removeButton = false;
  @Input() public editButton = false;
  @Input() public map: any;

  @Output()
  public buttonClicked: EventEmitter<
    INioTimelineButtonClick
  > = new EventEmitter<INioTimelineButtonClick>();
    ...
    if (!!this.items) {
      ...
      this.items.forEach((item: any, index: number) => {
        ...
        if (!periods.has(period)) {
          ...
          const periodDate = moment(date)[
            this.descending ? 'endOf' : 'startOf'
          ]('month');
